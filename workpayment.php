...
if (isset($_POST['submit'])) { 
   ...
   $date = $_POST['date'];
   $rem = $_POST['remarks'];
   $query="INSERT into workpayment (work_ref, reciept, cash, name, date, amount, remarks) values($wid,'$reciept',$cash,'$name','$date','$amount','$rem')";
...
if (isset($_POST['update'])) { 
	$wpid = $mysqli->escape_string($_POST['wpid']);
   ...
   $query="UPDATE workpayment set reciept='$reciept',amount='$amount',date='$date',remarks='$remarks' where wpid=$wpid";
   if(mysqli_query($connect,$query)){
		$_SESSION['alert']="Added Successfully";
		$_SESSION['code']=200;
   }
		...
		<div id="page-wrapper">	
						...
						<div class="form-three widget-shadow">
							...
							<input type="hidden" name="wid" value="<?php echo $wid;?>" required>
								<div class="form-group">
									...
									<div class="col-sm-8">
										YES&nbsp;&nbsp;<input type="radio" checked value="1" name="cash" required>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
										NO&nbsp;&nbsp;<input type="radio" value="0" name="cash" required>
									</div>
								</div>
								<div class="form-group">
									<label for="focusedinput" class="col-sm-2 control-label">Reciept No:</label>
									<div class="col-sm-8">
										<input type="text" name="reciept" class="form-control1" id="focusedinput" placeholder="Enter Reciept">
									</div>
								</div>
								<div class="form-group">
					...
					<p id="msg"></p>
						<table id="myTable" class="table">
							...
							<tbody>
								<?php
									$query = "SELECT * FROM workpayment where work_ref=".$wid."";  
								...
								{ 
								  ...
								  <td>'.$row['remarks'].'</td>
								  <td>
								  <input type="button" name="view" value="Edit" id="'.$row["wpid"].'" class="btn btn-primary  view_data" /> 
								  <form action="" method="post"><input type="hidden" onclick="return confirm(\'Are you sure you want to delete?\');" name="wdid" value=""><input type="submit" name="" class="btn btn-danger" value="delete"></form></td>
								  </tr>';
								}
								?>
							</tbody>
